...
class _FocusModeDockState extends State<FocusModeDock> {
  FocusModeType _currentMode = FocusModeType.focus;
  ...
  void initState() {
    super.initState();
    _applyModeDefaults(FocusModeType.focus);
  }
  ...
  void _applyModeDefaults(FocusModeType mode) {
    _timer?.cancel();
    _isRunning = false;
    _currentMode = mode;
  ...
  void _modifyMinutes(int delta) {
    ...
    setState(() {
      final currentMinutes = (_totalSeconds / 60).round();
  ...
  void _setMinutes(int minutes) {
    if (_isRunning) return;
    setState(() {
      _totalSeconds = minutes.clamp(1, 180) * 60;
      _remainingSeconds = _totalSeconds;
    });
  }
  ...
  void _onTimerComplete() {
    setState(() {
      _isRunning = false;
      _remainingSeconds = 0;
      if (_currentMode == FocusModeType.focus) {
        _completedSessions++;
      }
    });
    ...
    if (_currentMode == FocusModeType.focus) {
      widget.onToast('🎉 Great focus session completed! Time for a well-deserved break.');
    } else {
      widget.onToast('✨ Break over! Ready to focus again?');
    }
  }
