...
import 'package:zenbox/widgets/editor.dart' show askText;
...
class _DocumentEditorState extends State<DocumentEditor> {
  ...
  Future<void> _openNotesGridDialog() async {
    final picked = await showDialog<CreativeObject>(
      context: context,
      builder: (context) => NotesGridPickerModal(
        sources: widget.siblingDocuments,
        initialSelection: widget.object,
        title: 'All Notes & Documents',
      ),
    );
    if (picked != null && mounted) {
      widget.onSwitchDocument?.call(picked);
    }
  }
                    ...
                    children: [
                      ...
                      Expanded(
                        child: widget.siblingDocuments.isEmpty
                            ...
                            : Builder(
                                builder: (context) {
                                  final hasMore = widget.siblingDocuments.length > 10;
                                  final displayedDocuments = <CreativeObject>[];
                                  if (hasMore) {
                                    if (!widget.siblingDocuments.take(10).any((d) => d.id == widget.object.id)) {
                                      displayedDocuments.add(widget.object);
                                      displayedDocuments.addAll(
                                        widget.siblingDocuments.where((d) => d.id != widget.object.id).take(9),
                                      );
                                    } else {
                                      displayedDocuments.addAll(widget.siblingDocuments.take(10));
                                    }
                                  } else {
                                    displayedDocuments.addAll(widget.siblingDocuments);
                                  }
                                  ...
                                  return Row(
                                    children: [
                                              ...
                                              PopupMenuItem(
                                                ...
                                                child: Row(
                                                  children: [
                                                    ...
                                                    Expanded(
                                                      child: Text(
                                                        'See more… (${widget.siblingDocuments.length} in grid)',
                                                        style: TextStyle(
                                      ...
                                      if (widget.siblingDocuments.length > 1)
                                        IconButton(
                                          tooltip: 'Browse all notes in grid',
                                          padding: EdgeInsets.zero,
                                          constraints: const BoxConstraints(minWidth: 26, minHeight: 26),
                                          icon: Icon(Icons.grid_view_rounded, size: 16, color: sage),
                                          onPressed: _openNotesGridDialog,
                                        ),
                                    ],
                                  );
                                },
